...
n_data <- nitrogen_data %>% 
  mutate(percent_n_jan = percent_n_jan *100,
         ...
         percent_n_july = percent_n_july *100) %>% 
...
```{r}
# lidar layer 
bathy <- read.table(here("data/bathy_LIDAR.xyz")) %>% 
  as.data.frame()

bathy <- rename(bathy, longitude = V1)
bathy <- rename(bathy, latitude = V2) 
bathy <- rename(bathy, depth = V3) # threw an error when trying to do all the rename steps in one go 
...
bathy_raster_filtered <- raster(crs = crs(bathy_sf_filtered), vals = 0, resolution = c(20, 20), ext = extent(bathy_sf_filtered)) %>% 
  rasterize(bathy_points_filtered, ., field = bathy_points_filtered$depth)

plot(bathy_raster_filtered)
```
...
leaflet() %>% 
  ...
  addLegend(data = bathy_df, title = 'Depth', pal = pal_bathy, 
            position = "bottomright",
            values = ~layer, opactiy = 1, group = 'Depth')
