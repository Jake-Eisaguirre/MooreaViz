...
grd_100_sp <- as(shp_bound, "Spatial") %>% 
  st_as_sf() %>% 
  st_set_crs(4326) %>% 
  st_transform(crs)# converting to {sp} format

grd <- st_make_grid(grd_100_sp) %>% 
  st_set_crs(4326) %>% 
  st_transform(crs)

gridded(nut_bound) <- TRUE             # informing the object that it is a grid
