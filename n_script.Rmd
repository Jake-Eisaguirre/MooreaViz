...
grd_100_sf <- sf_jan_n_data %>% 
  ...
  cellsize = c(50, 50), # 100m pixel size
  what = "centers"
  ) %>%
...
bound_grid <- st_intersection(grd_100_sf, clipped_bound)
...
jan_krig <- krige(
  ...
  model =  # model fitted above !!! <---- Since the model is fitted by variogram and Tom did not do that, maybe we right
  )
