...
#vario gram and model selection - confirm ~1 and more clarity on variogram results
...
#What does the plateau tell us about the data?
plot(v_mod_full)


#making a gridded box with the bounds of the lat long in UTM - "centers" correct or should it be "polygons", "corners"?
grd_sf <- sf_jan_n_data %>% 
  st_bbox() %>% 
  st_as_sfc() %>% 
  st_make_grid(cellsize = c(20, 20), what = "centers") %>%
  st_as_sf() %>% 
  cbind(., st_coordinates(.)) 
...
#krige - any reason to not use our fitted model?
...
new_jan_ras <- raster(sf_bound, resolution(c(0.1,0.1)))

re_samp_jan_n <- resample(jan_ras, new_jan_ras, method = "bilinear")



clipped_ras <- mask(jan_ras, ras_bound)
...
leaflet() %>% 
  ...
  addRasterImage(jan_ras, colors = "plasma", group = "January N") %>% 
   addLayersControl(
...
saveWidget(base_leaf, file= "base_leaf.html", selfcontained = T)
