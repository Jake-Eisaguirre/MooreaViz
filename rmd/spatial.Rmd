...
bathy_raster_filtered <- raster(crs = crs, vals = 0, resolution = c(5, 5), ext = extent(sewage_ras)) %>% 
  rasterize(bathy_points_filtered, ., field = bathy_points_filtered$depth) %>% 
  projectRaster(sewage_ras, method = "bilinear")
...
spatial_brick <- brick(jan_ras, may_ras, july_ras, jan_ras_i, may_ras_i, july_ras_i, bleach_raster, sewage_ras, bathy_raster_filtered)
...
leaflet() %>% 
  ...
  addPolygons(data = lter_1) %>% 
  addPolygons(data = lter_2) %>% 
  addPolygons(data = lter_3) %>% 
  addPolygons(data = lter_4) %>% 
  addPolygons(data = lter_5) %>% 
  addPolygons(data = lter_6) 
