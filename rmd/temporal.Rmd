...
# plot showing all four variables for a chosen site
```{r}
# filter for just LTER 1 to start (would need to make a reactive dataframe on the app)
temporal_data_1 <- temporal_data %>% 
  filter(site == "LTER 1")

ggplot(data = temporal_data_1, aes(x = year)) + 
  geom_point(aes(y = mean_coral_cover, color = "mean_coral_cover")) +
  geom_line(aes(y = mean_coral_cover, color = "mean_coral_cover")) +
  geom_point(aes(y = mean_algae_cover, color = "mean_algae_cover")) +
  geom_line(aes(y = mean_algae_cover, color = "mean_algae_cover")) +
  geom_point(aes(y = mean_biomass_p_consumers, color = "mean_biomass_p_consumers")) +
  geom_line(aes(y = mean_biomass_p_consumers, color = "mean_biomass_p_consumers")) +
  geom_point(aes(y = cots_density, color = "cots_density")) +
  geom_line(aes(y = cots_density, color = "cots_density"))
```
